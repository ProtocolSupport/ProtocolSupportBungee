...
public class PipeLineBuilder extends IPipeLineBuilder {
	...
	public void buildBungeeClient(Channel channel, Connection connection) {
		...
		pipeline.replace(PipelineUtils.PACKET_DECODER, PipelineUtils.PACKET_DECODER, new FromClientDecoder(connection, cache));
		pipeline.replace(PipelineUtils.PACKET_ENCODER, PipelineUtils.PACKET_ENCODER, new ToClientEncoder(connection, cache));
	...
	public void buildBungeeServer(Channel channel, Connection connection) {
		...
		NetworkDataCache cache = new NetworkDataCache();
		pipeline.replace(PipelineUtils.PACKET_DECODER, PipelineUtils.PACKET_DECODER, new FromServerDecoder(connection, cache));
		pipeline.replace(PipelineUtils.PACKET_ENCODER, PipelineUtils.PACKET_ENCODER, new ToServerEncoder(connection, cache));
		pipeline.get(CustomHandlerBoss.class).setPacketHandlerChangeListener(listener -> {
			try {
