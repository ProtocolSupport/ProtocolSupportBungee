	...
	public static ByteBuf readPacket(int packetId, ByteBuf buf) {
		...
		switch (packetId) {
			case 0x05: { //EntityEquipment
				...
				packetdata.writeBytes(LegacySerializer.readItemStackData(buf));
				break;
			}
			...
			case 0x14: { //SpawnNamedEntity
				...
				LegacySerializer.writeString(LegacySerializer.readString(buf), packetdata);
				Utils.rewriteBytes(buf, packetdata, (Integer.BYTES * 3) + (Byte.BYTES * 2) + Short.BYTES);
				packetdata.writeBytes(LegacySerializer.readDatawatcherData(buf));
				break;
			}
