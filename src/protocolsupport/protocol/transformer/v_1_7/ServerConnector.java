	...
	public static void connect(final BungeeCord bungee, final UserConnection connection, final ServerInfo info, final boolean retry) {
		...
		ChannelInitializer<Channel> initializer = new ChannelInitializer<Channel>() {
			protected void initChannel(Channel ch) throws Exception {
				...
				ch.pipeline().replace(PipelineUtils.FRAME_DECODER, PipelineUtils.FRAME_DECODER, new FrameDecoder());
				ch.pipeline().addAfter(PipelineUtils.FRAME_DECODER, PipelineUtils.PACKET_DECODER, new MinecraftDecoder(Protocol.HANDSHAKE, false, connection.getPendingConnection().getVersion()));
				ch.pipeline().addAfter(PipelineUtils.FRAME_PREPENDER, PipelineUtils.PACKET_ENCODER, new MinecraftEncoder(Protocol.HANDSHAKE, false, connection.getPendingConnection().getVersion()));
				ch.pipeline().get(HandlerBoss.class).setHandler(new net.md_5.bungee.ServerConnector(bungee, connection, target));
			}
		};
